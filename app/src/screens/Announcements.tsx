...
import { AudiencePicker, Confirm, Empty, PriorityChip, Sheet, Skeleton, SkeletonList, audienceLabel, type Audience } from '../components/ui';
...
function StatusChip({ a }: { a: Announcement }) {
  if (a.status === 'draft')
    return (
      <span className="chip">
        <EditIcon style={{ width: 12, height: 12 }} /> Draft
      </span>
    );
  if (a.status === 'scheduled')
    return (
      <span className="chip warn">
        <ClockIcon style={{ width: 12, height: 12 }} /> Scheduled · {formatDateTime(a.publish_at!)}
      </span>
    );
...
export function AnnouncementsScreen() {
  ...
  // The server pages the list (30 at a time) so a long history doesn't have to be downloaded to read the latest.
  const [extra, setExtra] = useState<Announcement[]>([]);
  const [loadingMore, setLoadingMore] = useState(false);
  const { data, error, loading } = useLoader(async () => {
    setExtra([]);
    return api.announcements({ ...query, limit: PAGE });
  ...
  const items = [...(data?.announcements || []), ...extra];
  const total = data?.total ?? items.length;
  const loadMore = async () => {
    setLoadingMore(true);
    try {
      const r = await api.announcements({ ...query, limit: PAGE, offset: items.length });
      setExtra((x) => [...x, ...r.announcements]);
    } finally {
      ...
      {items.length < total && (
        <div className="row" style={{ justifyContent: 'center', marginTop: 14 }}>
          <button className="btn" onClick={loadMore} disabled={loadingMore}>
            {loadingMore ? 'Loading…' : `Show more (${(total - items.length).toLocaleString()} older)`}
          </button>
        </div>
      )}
    ...
    <div className={`card clickable ${!a.read_by_me && !isAdmin ? 'unread' : ''}`} onClick={onOpen} style={{ opacity: a.status === 'expired' ? 0.7 : 1 }}>
      <div className="row between" style={{ alignItems: 'flex-start' }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div className="row wrap" style={{ gap: 6, marginBottom: 4 }}>
            {a.pinned && (
              <span className="chip">
                <PinIcon style={{ width: 12, height: 12 }} /> Pinned
              </span>
            )}
        ...
        {!isAdmin && a.read_by_me && (
          <span className="chip ok">
            <CheckIcon style={{ width: 12, height: 12 }} /> Read
          </span>
        )}
      </div>
      <div className="row between tiny muted wrap" style={{ marginTop: 10 }}>
        <span>
          ...
          {(a.attachment_count ?? 0) > 0 && (
            <>
              {' '}
              · <LinkIcon style={{ width: 12, height: 12, verticalAlign: '-2px' }} /> {a.attachment_count} file{a.attachment_count > 1 ? 's' : ''}
            </>
          )}
          {(a.comment_count ?? 0) > 0 && (
            <>
  ...
  return (
    <div className="card">
      ...
      <p className="tiny muted" style={{ marginBottom: 10 }}>
        ...
        {canVote && !poll.my_vote ? ' · tap an option to vote' : ''}
      </p>
      {poll.options.map((o) => {
        ...
        return (
          <button key={o.id} type="button" className={`poll-option ${poll.my_vote === o.id ? 'chosen' : ''}`} onClick={() => vote(o.id)} disabled={!canVote}>
            ...
            <span className="lbl">
              <span>
                {poll.my_vote === o.id && <CheckIcon style={{ width: 14, height: 14, verticalAlign: '-2px', color: 'var(--primary)' }} />} {o.label}
              </span>
...
export function AnnouncementDetail({ id }: { id: number }) {
  ...
  return (
    <>
      <div className="card">
        ...
        <p className="muted small" style={{ marginBottom: 16 }}>
          ...
          {a.expires_at && (
            <>
              {' '}
              · {a.status === 'expired' ? 'expired' : 'expires'} {formatDateTime(a.expires_at)}
            </>
          )}
        </p>
        ...
        {viewerOnly &&
          ...
          (a.acked_by_me ? (
            <div className="success" style={{ marginTop: 18 }}>
              <CheckIcon style={{ width: 16, height: 16, verticalAlign: '-3px' }} /> You acknowledged this announcement.
            </div>
          ) : (
            <div style={{ marginTop: 18 }}>
              <div className="error" style={{ background: 'var(--warn-soft)', color: 'var(--warn)', marginBottom: 10 }}>
                Management asks you to confirm you've read and understood this.
              </div>
              <button className="btn primary block" onClick={acknowledge}>
                <CheckIcon /> I have read and understood this
              </button>
            </div>
          ))}
        ...
        {isAdmin && (
          <div className="row wrap" style={{ marginTop: 18, justifyContent: 'flex-end' }}>
            {a.status === 'draft' && (
              <button className="btn sm primary" onClick={() => (a.body.trim() ? setConfirmPublish(true) : setEdit(true))}>
                <CheckIcon /> Publish
              </button>
            )}
            ...
            <button className="btn sm" onClick={() => setDuplicate(true)} title="Start a new announcement with the same content">
              <CopyIcon /> Duplicate
            </button>
            <button className="btn sm" onClick={() => setSaveTpl(true)} title="Save as a reusable template">
      ...
      {a.poll && (
        <PollBox announcementId={a.id} poll={a.poll} canVote={viewerOnly && a.status === 'live'} onVoted={(poll) => setData({ announcement: { ...a, poll, read_by_me: true } })} />
      )}
        ...
        <div className="card">
          <div className="row between">
            ...
            <span className="chip primary">
              {a.read_count}/{a.audience_count} · {pct}%
            </span>
          </div>
          ...
          {a.ack_required && (
            <p className="small muted" style={{ marginBottom: 6 }}>
              Acknowledged: <strong>{a.ack_count}</strong> of {a.audience_count}
            </p>
          )}
          ...
          <div className="list">
            ...
            {(a.readers_total ?? 0) > (a.readers?.length ?? 0) && (
              <p className="tiny muted" style={{ padding: '8px 0' }}>
                …and {((a.readers_total ?? 0) - (a.readers?.length ?? 0)).toLocaleString()} more (see Reports for the full list).
              </p>
            )}
          </div>
          ...
          <div className="list">
            ...
            {(a.unread_total ?? 0) > (a.unread?.length ?? 0) && (
              <p className="tiny muted" style={{ padding: '8px 0' }}>
                …and {((a.unread_total ?? 0) - (a.unread?.length ?? 0)).toLocaleString()} more (see Reports for the full list).
              </p>
            )}
          </div>
        </div>
      ...
      {confirmPublish && (
        <Confirm
          ...
          message={
            a.publish_at && a.publish_at > new Date().toISOString()
              ? `It will go out at ${formatDateTime(a.publish_at)} and employees will be notified then.`
              : 'Everyone it is sent to gets a notification right away.'
          }
...
function SaveTemplateSheet({
  ...
  a: { title: string; body: string; priority: Announcement['priority']; category: string | null; ack_required: boolean; poll: Poll | null };
  onClose: () => void;
}) {
  const { toast } = useStore();
    ...
    <Sheet title="Save as template" onClose={onClose}>
        ...
        onSubmit={async (e) => {
          ...
          try {
            await api.createTemplate({
              ...
              poll_question: a.poll?.question || null,
              poll_options: a.poll?.options.map((o) => o.label) || [],
              company_id: null,
            });
      ...
      >
        ...
        <p className="small muted">
          Templates keep the title, message, priority, topic, acknowledgement and poll — you can pick them from "Use a template" when writing a new announcement.
        </p>
        <div className="field">
          <label>Template name</label>
          <input className="input" value={name} onChange={(e) => setName(e.target.value)} required autoFocus placeholder="e.g. Holiday notice" />
        </div>
    ...
    <Sheet title={existing ? (isDraft ? 'Edit draft' : 'Edit announcement') : prefill ? 'Duplicate announcement' : 'New announcement'} onClose={onClose}>
      <form className="stack" onSubmit={(e) => submit(e, false)}>
        ...
        <FilePicker
          files={files}
          onChange={setFiles}
          existing={(existing?.attachments ?? []).filter((a) => !removeIds.includes(a.id))}
          onRemoveExisting={(id) => setRemoveIds((r) => [...r, id])}
        />
        ...
        <label className="check">
          <input type="checkbox" checked={ackRequired} onChange={(e) => setAckRequired(e.target.checked)} /> Require acknowledgement ("I have read and understood")
        </label>
        <label className="check">
          <input type="checkbox" checked={schedule} onChange={(e) => setSchedule(e.target.checked)} /> Publish later
        </label>
        {schedule && (
        ...
        <label className="check">
          <input type="checkbox" checked={expire} onChange={(e) => setExpire(e.target.checked)} /> Hide automatically after a date
        </label>
        {expire && (
          <div className="field" style={{ paddingLeft: 26 }}>
        ...
        <label className="check">
          <input type="checkbox" checked={hasPoll} onChange={(e) => setHasPoll(e.target.checked)} /> Add a quick poll
        </label>
        {hasPoll && (
          <div className="stack" style={{ paddingLeft: 26 }}>
            ...
            <div className="field">
              ...
              {pollOptions.map((o, i) => (
                <div className="row" key={i}>
                  ...
                  {pollOptions.length > 2 && (
                    <button type="button" className="btn ghost icon-btn" onClick={() => setPollOptions(pollOptions.filter((_, j) => j !== i))}>
                      <TrashIcon />
                    </button>
                  )}
                </div>
              ...
              {pollOptions.length < 10 && (
                <button type="button" className="btn sm" style={{ alignSelf: 'flex-start' }} onClick={() => setPollOptions([...pollOptions, ''])}>
                  + Add option
                </button>
              )}
        ...
        <div className="row" style={{ justifyContent: 'center', gap: 6 }}>
          {(!existing || isDraft) && (
            <button className="btn sm" type="button" disabled={busy} onClick={() => submit(null, true)}>
              <SaveIcon /> {isDraft ? 'Save draft' : 'Save as draft'}
            </button>
          )}
          <button className="btn sm ghost" type="button" onClick={() => setSaveTpl(true)} disabled={!title.trim()}>
            Save as template
          </button>
        </div>
        {!existing && !schedule && (
          <p className="tiny muted" style={{ textAlign: 'center' }}>
            Everyone it's sent to gets a notification right away. Drafts are only visible to you and other managers.
          </p>
        )}
      </form>
      {saveTpl && (
        <SaveTemplateSheet
          a={{
            ...
            category: category.trim() || null,
            ack_required: ackRequired,
            poll: hasPoll ? { question: pollQuestion, options: pollOptions.filter(Boolean).map((label, i) => ({ id: i, label, votes: 0 })), my_vote: null, total: 0 } : null,
          }}
          onClose={() => {
            setSaveTpl(false);
            reloadTemplates();
...
function TemplateManager({ templates, onChanged }: { templates: Template[]; onChanged: () => void }) {
  ...
  return (
    <>
      <button type="button" className="btn" onClick={() => setOpen(true)} title="Manage templates">
        <EditIcon />
      </button>
      {open && (
