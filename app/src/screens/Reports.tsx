...
import { api, formatDateTime, openProtectedFile, type ReportSummary } from '../api';
...
export function ReportsScreen() {
  ...
  const open = async (kind: View) => {
    try {
      await openProtectedFile(api.reportCsvPath(kind, from || undefined, to || undefined));
    } catch (err) {
    ...
    <>
      <div className="card" style={{ marginBottom: 12 }}>
        <div className="row wrap" style={{ alignItems: 'flex-end' }}>
          ...
          <button className="btn primary" onClick={() => open(view)}>
            <LinkIcon /> Download CSV
          </button>
        </div>
        <p className="tiny muted" style={{ marginTop: 8 }}>
          CSV files open in Excel. The download uses the current tab and date range.
        </p>
      </div>

      {loading && <Skeleton card lines={3} />}
      {error && <div className="error">{error}</div>}
      {data && (
        <>
          <div className="grid-3 stagger" style={{ marginBottom: 14 }}>
            <div className="stat">
              <div className="num">
                <CountUp value={data.totals.announcements} />
              </div>
              <div className="lbl">Announcements</div>
            </div>
            <div className="stat">
              <div className="num">
                <Pct v={data.totals.avg_read_pct} />
              </div>
              <div className="lbl">Average read rate</div>
            </div>
            <div className="stat">
              <div className="num">
                <CountUp value={data.totals.meetings} />
              </div>
              <div className="lbl">Meetings</div>
            </div>
            <div className="stat">
              <div className="num">
                <Pct v={data.totals.avg_going_pct} />
              </div>
              <div className="lbl">Average "going" rate</div>
            </div>
            <div className="stat">
              <div className="num">
                <Pct v={data.totals.avg_attended_pct} />
              </div>
              <div className="lbl">Average actual attendance</div>
            </div>
          </div>
          ...
          <div className="row between wrap" style={{ marginBottom: 10 }}>
            <div className="seg">
              ...
              <button className={view === 'employees' ? 'active' : ''} onClick={() => setView('employees')}>
                Employees
              </button>
    ...
    <table className="report">
      <thead>
        <tr>
          ...
          <th className="num">Announcements sent</th>
      ...
      <tbody>
        ...
        {d.groups.map((g, i) => (
          <tr key={i}>
            <td>{g.company}</td>
            <td>{g.department}</td>
            <td className="num">{g.employees}</td>
            <td className="num">{g.sent}</td>
            <td className="num">{g.read}</td>
            <td className="num">
              <Pct v={g.read_pct} />
            </td>
            <td className="num">{g.invited}</td>
            <td className="num">{g.going}</td>
            <td className="num">
              <Pct v={g.going_pct} />
            </td>
            <td className="num">{g.attended}</td>
            <td className="num">
              <Pct v={g.attended_pct} />
            </td>
          </tr>
        ))}
      </tbody>
        ...
        {rows.map((e) => (
          <tr key={e.id}>
            <td>
              ...
              <div className="tiny muted">{e.email}</div>
            </td>
            <td className="small">{[e.company, e.department].filter(Boolean).join(' · ') || '–'}</td>
            <td className="num">{e.sent}</td>
            <td className="num">{e.read}</td>
            <td className="num">
              <Pct v={e.read_pct} />
            </td>
            <td className="num">{e.ack_required ? `${e.acked}/${e.ack_required}` : '–'}</td>
            <td className="num">{e.invited}</td>
            <td className="num">{e.going}</td>
            <td className="num">{e.maybe}</td>
            <td className="num">{e.declined}</td>
            <td className="num">{e.no_reply}</td>
            <td className="num">
              <Pct v={e.attendance_pct} />
            </td>
            <td className="num">{e.attended}</td>
            <td className="num">
              <Pct v={e.attended_pct} />
            </td>
          </tr>
        ))}
      </tbody>
    ...
    <table className="report">
      <thead>
        <tr>
          ...
          <th className="num">Read</th>
          <th className="num">Read %</th>
          <th className="num">Acknowledged</th>
        </tr>
      </thead>
      <tbody>
        ...
        {d.announcements.map((a) => (
          <tr key={a.id} style={{ cursor: 'pointer' }} onClick={() => onOpen(a.id)}>
            ...
            <td>
              ...
              {a.priority !== 'normal' && (
                <span className={`chip ${a.priority === 'urgent' ? 'danger' : 'warn'}`} style={{ marginLeft: 6 }}>
                  {a.priority}
                </span>
              )}
            </td>
            ...
            <td className="num">{a.audience}</td>
      ...
      <tbody>
        ...
        {d.meetings.map((m) => (
          <tr key={m.id} style={{ cursor: 'pointer' }} onClick={() => onOpen(m.id)}>
            ...
            <td className="num">{m.audience}</td>
            <td className="num">{m.going}</td>
            <td className="num">{m.maybe}</td>
            <td className="num">{m.declined}</td>
            <td className="num">{m.noReply}</td>
            <td className="num">
              <Pct v={m.going_pct} />
            </td>
            <td className="num">{m.attended}</td>
            <td className="num">
              <Pct v={m.attended_pct} />
            </td>
