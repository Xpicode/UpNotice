...
| POST | /api/auth/2fa/setup `{method: 'app'|'email'}` · /2fa/send-code · /api/auth/login/2fa/resend `{twofa_token}` · /2fa/enable `{code}` · /2fa/disable `{password,code}` · /2fa/recovery-codes `{code}` · /api/users/:id/2fa/reset | signed in · signed in · anyone · signed in · signed in · signed in · admin |
