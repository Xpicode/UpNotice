...
For a real release, bake the address in instead so nobody has to type it:

```bash
VITE_SERVER_URL=https://notice.yourcompany.com npm run mobile:android
```

The same variable works for `npm run build` (web) and `npm run desktop:build` (Windows). Anyone can still
change it under **Server** on the sign-in screen; it only sets the starting point.
...
## Running a pilot with one department

The system has never carried real company traffic. Do not switch everyone over on day one — run it with a
single department for two weeks first, where a problem is a conversation and not an incident.

1. **Check the deployment before anyone signs in.** From the `pro` folder:
   ```bash
   npm run preflight https://notice.yourcompany.com
   ```
   It reads `server/.env` and asks the live server, then reports `PASS` / `WARN` / `FAIL` for the signing key,
   demo accounts, CORS, HTTPS, the proxy setting, the database, email, push, the security headers, and whether
   the published demo password still opens the door. It changes nothing and exits non-zero if there is a
   blocker, so it can gate a release. Run it again after every settings change.
2. **Import only the pilot department.** People → Import with just those staff. Everyone else has no account,
   so there is nothing for them to sign in to and nothing to undo if you stop.
3. **Target everything at that department** while the pilot runs: the audience picker on an announcement or a
   meeting takes a company and a department, so nothing reaches anyone outside it.
4. **Tell them who to complain to**, by name. A pilot with no route for "this looks wrong" only tells you that
   nobody spoke up.
5. **Restore from a backup once, on purpose,** before the second week. A backup you have not restored from is
   not a backup.
6. **Watch three things**: the Activity log (admin-only) for failed sign-ins and anything odd, whether people
   actually read announcements (Reports), and whether check-ins get approved or quietly ignored.
7. **Then widen it** one department at a time, not all at once.
...
4. Back up the `upnotice-pgdata` and `upnotice-data` volumes — and restore from one once to prove it works.
5. Run `npm run preflight https://your-address` and clear every blocker before the first person signs in.
...
| Method | Path | Who |
|--------|------|-----|
| POST | /api/auth/login → `{token, refresh_token, expires_in, user}`, or `{twofa_required, twofa_token}` when two-factor is on · /api/auth/login/2fa `{twofa_token, code}` · /api/auth/refresh `{refresh_token}` (rotates) · /api/auth/logout · /api/auth/logout-all · GET /api/auth/sessions | all · signed in |
| POST | /api/auth/2fa/setup · /2fa/enable `{code}` · /2fa/disable `{password,code}` · /2fa/recovery-codes `{code}` · /api/users/:id/2fa/reset | signed in · signed in · signed in · signed in · admin |
