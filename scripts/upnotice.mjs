...
function run(cmd, args, opts = {}) {
  return spawnSync(cmd, args, { cwd: root, encoding: 'utf8', shell: win, windowsHide: true, ...opts });
}
function ok(result) {
...
function killPort(port) {
  if (win) {
    const out = run('netstat', ['-ano'], { stdio: 'pipe' }).stdout || '';
    const pids = new Set();
    ...
    for (const pid of pids) run('taskkill', ['/PID', pid, '/F', '/T'], { stdio: 'ignore' });
    return pids.size;
  }
...
function ensureInstalled(dir, label) {
  ...
  log(`Installing ${label} packages (first time only)...`);
...
function openBrowser(url) {
  if (win) run('cmd', ['/c', 'start', '""', url], { stdio: 'ignore', shell: false });
...
function startChild(label, color, cmd, args, opts) {
  // On macOS/Linux the child gets its own process group so we can stop npm + node together.
  const child = spawn(cmd, args, { shell: win, windowsHide: true, detached: !win, stdio: ['ignore', 'pipe', 'pipe'], ...opts });
...
function killChild(child) {
  ...
  if (win) run('taskkill', ['/PID', String(child.pid), '/T', '/F'], { stdio: 'ignore', shell: false });
  else {
    try {
...
async function dev() {
  ...
  const api = startChild('api', c.magenta, 'npm', ['run', 'dev'], { cwd: serverDir, env });
