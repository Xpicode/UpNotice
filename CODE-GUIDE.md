...
- **`app/`** is what people see: login, announcements, meetings, notifications. It never touches the database directly — it only *asks* the server, e.g. "give me the announcements" or "mark this one as read".
...
| `src/totp.js` | **Two-factor authentication.** Time-based one-time passwords (RFC 6238) written with `node:crypto` rather than an npm package — base32, `generateSecret`, `verifyCode` (±1 step, and it returns the step so the same code cannot be used twice), `otpauthUrl` for the QR, `encryptSecret`/`decryptSecret` (AES-256-GCM with a key derived from `JWT_SECRET`, so a database dump alone is not enough), and the single-use recovery codes. Unit-tested against the RFC's own published vectors. |
...
| `src/notify.js` | `createNotifications(userIds, {...})`: writes a row in `notifications` for each person, pushes it live over SSE, sends a mobile push **and an email** (link `?open=announcement:12` opens the item directly). Every "someone should be told" moment in the server calls this. `managerIds(companyId)` = the admins plus that company's managers, used when an employee reads/acks/comments. `purgeOldNotifications()` runs once a day from the scheduler and clears read notifications after 90 days and unread ones after 180 — this is the fastest-growing table and nothing else prunes it. |
| `src/events.js` | The SSE hub: remembers which browsers/phones are connected and lets the server say "announcements changed" to them. It counts the open connections per **session**, so `isSessionLive(sessionId)` can tell Settings which signed-in devices have the app open right now. |
...
| `src/screens/Meetings.tsx` | List (upcoming/past, search/filters, **List / Calendar** toggle) + detail: RSVP buttons and reason box, Google Calendar link, employee **check-in** box, staff **attendance card** (code + QR, "Present" tick per attendee), **minutes** editor + create/edit sheet with repeat options and Duplicate. |
| `src/screens/Notifications.tsx` | The Notifications tab: list, open, mark all read, trash per row, Select mode, clear read / clear all. |
...
| `recovery_codes` | single-use codes for a lost phone | `user_id`, `code_hash` (SHA-256), `used_at` |
| `users` | a person who can sign in | `email`, `password_hash`, `role` (`admin` / `manager` / `employee`), `company_id`, `department_id`, `active`, `avatar_path`, `email_notifications`, `must_change_password` (1 until a temporary password is replaced), `totp_secret` (encrypted) / `totp_enabled` / `totp_last_step` (two-factor authentication) |
...
| `notifications` | one notification for one user | `title`, `body`, `ref_type`/`ref_id` (what it points to), `read_at` |
...
10. **Back in the app** — the sheet closes and `toast('Announcement posted')`. Meanwhile, on every employee's device, `store.tsx` received the SSE event and bumped `version`; `useLoader` in their Announcements and Notifications screens re-fetches, so the new item appears within a second — no refresh. `notify.ts` also shows a system notification.
...
| change how long notifications are kept (90 days read / 180 unread) | `READ_NOTIFICATION_DAYS` / `UNREAD_NOTIFICATION_DAYS` in `server/src/notify.js` |
