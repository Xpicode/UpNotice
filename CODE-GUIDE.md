...
| `src/components/checkin.tsx` | Everything the attendee sees for attendance: `CheckInBanner` (the strip across the top of every screen while a meeting they are expected at is starting — it reads `dashboard.openCheckIn`), `CheckInPanel` (the "Attendance" section on the meeting page) and `CheckInForm` (the code box itself). All of them read the clock through `useNow`, so the box opens and closes by itself. |
...
| change the check-in time window (30 min before / 2 h after) | `CHECKIN_OPENS_BEFORE_MS` / `CHECKIN_CLOSES_AFTER_MS` in `server/src/routes/meetings.js` — the app follows the server |
